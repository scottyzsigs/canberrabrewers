...
<?php
	...
	switch ($formaction) {
		case 'Send email to members':
			...
			$premsg = '<style type="text/css">body, h1, h2, h3  {font-family:Arial, "Helvetica Neue", Helvetica, sans-serif; } p, td, div {font-family:Arial, "Helvetica Neue", Helvetica, sans-serif; }</style>';
			$premsg .= !empty($_POST['emailmsg']) ? $_POST['emailmsg'] : '';
			$msg = $premsg;
				...
				else
				{
					...
					while ($row = mysqli_fetch_array($user_result))
					{
						if($emailaction != '')
						{
							...
							$msg = str_replace("[member_firstname]", $mf_name, $premsg);
						}
						if($row['user_email'] != '')
						{
							echo "got record";
							if(!$debug_mode)
							{
								...
								{
									$sql = "UPDATE forumv3_users SET user_type=1, user_inactive_reason=3 WHERE user_id=?";
									if ($msql = $mysqli->prepare($sql)) 
									{
										$msql->bind_param('i',$row['user_id']);
										//$msql->execute();
									}
									$sql = "UPDATE cb_membership SET member_disabled_year=? WHERE member_id=?";
									if ($msql = $mysqli->prepare($sql)) 
									{
										$msql->bind_param('ii',$_GET['year'],$row['member_id']);
										//$msql->execute();
									}
									echo "Disabled ".$row['member_id']." forum id: ".$row['user_id'];
								}
								else
								{
									if($emailaction == "youfuckenidiot")
			...
			break;
		case 'disablemember':
			if ($msql = $mysqli->prepare("UPDATE forumv3_users SET user_type=1, user_inactive_reason=3 WHERE user_id=?")) 
			{
				$msql->bind_param('i',$_POST['user_id']);
				$msql->execute();
			}
			if ($msql = $mysqli->prepare("UPDATE cb_membership SET member_disabled_year=? WHERE member_id=?")) 
			{
				$msql->bind_param('ii',$_POST['year'],$_POST['member_id']);
				$msql->execute();
			}
			echo "Disabled ".$_POST['member_id']." forum id: ".$_POST['user_id'];
			break;
		case 'enablemember':
			$sql = "UPDATE forumv3_users SET user_type=0, user_inactive_reason=0 WHERE user_id=?";
			if ($msql = $mysqli->prepare($sql)) 
			...
			if ($msql = $mysqli->prepare($sql)) 
			{
				$msql->bind_param('i',$_POST['member_id']);
				$msql->execute();
			}
			echo "Enabled ".$_POST['member_id'];
			break;
}
switch ($action) {
	case 'emailform':
...
// end email form
	...
	{
	// change to 2017 to see history back that far
	for ($x = $thisyear; $x >= 2018; $x--) {
	...
	$members = '';
	//unpaid members
	$memberlistsql = "SELECT member_id, member_firstname, member_surname, member_forum_name, member_email, cb_membership.member_id, username, user_id FROM cb_membership INNER JOIN forumv3_users ON username = member_forum_name WHERE (member_disabled_year = 0) AND (user_type = 0 or user_type = 3) AND NOT EXISTS (SELECT * FROM cb_membership_transactions WHERE cb_membership_transactions.member_id = cb_membership.member_id AND (DATE(`transaction_date`) BETWEEN '".$qdate."' AND '".$qedate."')) ORDER BY member_firstname, member_surname"; 
	$member_result=$mysqli->query($memberlistsql);
	$num_rows = mysqli_num_rows($member_result);
	while ($row = mysqli_fetch_array($member_result))
	{
		$members .= '<tr><td>'.$row['member_firstname'].' '. $row['member_surname']. ' <span title="Member ID: '.$row['member_id'].' Actual forum name: '.$row['username'].'">('.$row['member_forum_name'].')</span></td><td>'. $row['member_email'] .'</td><td>Unpaid for '. $x .'</td><td><form method="post"><input type="hidden" name="formaction" value="disablemember" /><input type="hidden" name="member_id" value="' . $row['member_id'] . '" /><input type="hidden" name="user_id" value="' . $row['user_id'] . '" /><input type="hidden" name="year" value="' . $x . '" /><input type="submit" value="Disable forum login" /></form></td></tr>';
	}
	$tbldata = sprintf($tbl,$members);
	echo '<h2>'.$x.' Unpaid Members ('.$num_rows.') | <a href="membership_admin.php?action=emailform&emailaction=members&year='.$x.'">Email unpaid '.$x.' members</a> | <a href="membership_admin.php?action=emailform&emailaction=disablemembers&year='.$x.'">DISABLE unpaid '.$x.' members</a></h2>'.$tbldata;
	$members = '';
	// disbaled members who are in membership table (from 2018)
	$memberlistsql = "SELECT member_id, member_firstname, member_surname, member_forum_name, member_email, cb_membership.member_id, username, user_id FROM cb_membership INNER JOIN forumv3_users ON username = member_forum_name WHERE (user_type = 1 AND user_inactive_reason = 3) AND (member_disabled_year = ". $x .") ORDER BY member_firstname, member_surname"; 
	$member_result=$mysqli->query($memberlistsql);
	$num_rows = mysqli_num_rows($member_result);
	while ($row = mysqli_fetch_array($member_result))
	{
		$members .= '<tr><td>'.$row['member_firstname'].' '. $row['member_surname']. ' <span title="Member ID: '.$row['member_id'].' Actual forum name: '.$row['username'].'">('.$row['member_forum_name'].')</span></td><td>'. $row['member_email'] .'</td><td>Unpaid for '. $x .'</td><td><form method="post"><input type="hidden" name="formaction" value="enablemember" /><input type="hidden" name="member_id" value="' . $row['member_id'] . '" /><input type="hidden" name="user_id" value="' . $row['user_id'] . '" /><input type="submit" value="Enable forum login" /></form></td></tr>';
	}
	$tbldata = sprintf($tbl,$members);
	echo '<h2>'.$x.' DISABLED Members ('.$num_rows.') </h2>'.$tbldata;
	$members = '';
	// end for
	}
